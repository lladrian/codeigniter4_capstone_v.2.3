...
class RegistrarController extends BaseController
{
    ...
    public function __construct()
   {
    ...
    {  
        ...
        $db = $this->service->initializeDatabase('registrars', 'id');
...
// exit;
       ...
       return view('/tables/registrar_table/registrars_table', $data);
    }
    ...
    public function edit_registrar($id) {
        ...
        if ($data['user']) {
           return view('/tables/registrar_table/edit_registrar',$data);
        } else {
            return redirect()->to('/tables/dashboard/registrars')->with('error', 'User not found.');
        }
    }
    ...
    public function add_registrar()
    {
        ...
        // Load dashboard view
        return view('/tables/registrar_table/add_registrar');
    }
 ...
 //exit;
        if (!$validation->withRequest($this->request)->run()) {
            ...
            if ($db->insert($userData)) {
                return redirect()->to('/dashboard/registrars')->with('success', 'Feature added successfully.');
            } else {
                return redirect()->to('/dashboard/registrars')->with('error', 'Failed to add feature.');
            }
        }
    }

    
    public function update_registrar($id)
    {
        $validation = \Config\Services::validation();
        $request = service('request');
        ...
        if (!$validation->withRequest($this->request)->run()) {
            ...
            if ($db->update($id, $userData)) {
                return redirect()->to('/dashboard/registrars')->with('success', 'Feature updated successfully.');
            } else {
                return redirect()->back()->withInput()->with('error', ['Failed to update feature.']);
            }
            
        }
        return redirect()->to('/dashboard/registrars')->with('errors', ['Failed to update feature.']);
    }
    ...
    public function delete_registrar($id) {
        ...
        $countResult = $db->createCustomQuery($query)->getResult();
         ...
         if ($countResult[0]->count == 0) {
            ...
            return redirect()->to('/dashboard/registrars')->with('error', 'Feature not found.');
        }

        if ($db->delete($id)) {
            // Deletion successful, redirect with success message
            return redirect()->to('/dashboard/registrars')->with('success', 'Feature deleted successfully.');
        } else {
            // Deletion failed, handle error (e.g., show an error message)
            return redirect()->to('/dashboard/registrars')->with('error', 'Failed to delete feature.');
        }   
    }
